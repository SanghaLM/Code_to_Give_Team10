...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Hardcoded demo credentials for teacher and parent
      if (normalizedUsername === "admin" && password === "admin") {
        console.log("Teacher demo login successful");
        await AsyncStorage.multiSet([
          ["userToken", "local-teacher-token"],
          [
            "userData",
            JSON.stringify({
              username: "admin",
      ...
      if (normalizedUsername === "sarahchen" && password === "1234") {
        console.log("Parent demo login successful");
        const demoChild = {
          _id: "local-child-1",
          firstName: "Emma",
          lastName: "Cheung",
          kindergartenLevel: "K1",
          kindergartenName: "Demo Kindergarten",
        };
        await AsyncStorage.multiSet([
          ["userToken", "local-parent-token"],
          [
            "userData",
            JSON.stringify({
              username: "sarahchen",
      ...
      if (response.ok) {
        ...
        Alert.alert(
          "Login Failed",
          data.message ||
            "Invalid credentials. Use admin/admin for teacher or sarahchen/1234 for parent."
        );
      }
    } catch (error) {
      console.error("Login error:", error.message);
